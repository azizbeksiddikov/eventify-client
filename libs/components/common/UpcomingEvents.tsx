...
const UpcomingEvents = ({ events, organizerName, likeEventHandler }: UpcomingEventsProps) => {
	...
	if (events.length === 0) {
		return (
			<Card className="mt-10 p-6 sm:p-8 bg-card hover:bg-accent/5 transition-all duration-300 shadow-sm hover:shadow-md border border-border/50 text-center">
				{title}
	...
	return (
		<Card className="mt-10 p-6 sm:p-8 bg-card hover:bg-accent/5 transition-all duration-300 shadow-sm hover:shadow-md border border-border/50">
			{title}
			<div className="mt-6">
