...
const CommentsComponent = ({ comments: initialComments, commentGroup, commentRefId }: CommentsProps) => {
	...
	return (
		<div className="rounded-xl bg-card/50 shadow-sm mt-5 border border-border/50">
			<div className="p-8">
				<div className="flex items-center justify-between mb-6">
					<h3 className="text-xl font-semibold text-foreground/90">Comments</h3>
					<div className="flex items-center space-x-4">
						...
						{!isWriting && (
							<Button
								...
								className="text-muted-foreground/80 hover:text-foreground transition-colors border-border/50"
							>
							...
							<div className="flex items-start space-x-4">
								<div className="flex-1 space-y-3">
									<Textarea
										...
										className="min-h-[100px] resize-none text-sm bg-card/50 border-border/50"
									/>
				...
				{totalComments <= 0 && !isWriting ? (
					<div className="text-center py-12">
						<div className="text-muted-foreground/80 mb-4 text-sm">No comments yet</div>
						<Button
							variant="outline"
							onClick={() => setIsWriting(true)}
							className="text-muted-foreground/80 hover:text-foreground transition-colors border-border/50"
						...
						{comments.map((comment, index) => {
							...
							return (
								<div key={comment._id}>
									...
									{index < comments.length - 1 && <Separator className="my-6 bg-border/50" />}
								</div>
							);
						})}
					</div>
