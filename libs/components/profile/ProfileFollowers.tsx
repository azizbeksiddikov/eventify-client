...
import { User, Heart, UserPlus, UserMinus, Users, Calendar } from "lucide-react";
			...
			<div className="p-6">
				{followers.length === 0 ? (
					...
					<Table>
						...
						<TableBody>
							{followers.map((user) => {
								const meFollowed = user?.meFollowed?.[0]?.myFollowing;
												...
												<Link href={`/organizer/detail?organizerId=${user._id}`}>
													<div className="flex items-center gap-4 underline">
														<Avatar className="h-10 w-10">
															{user.memberImage ? (
																<AvatarImage
																	src={`${NEXT_APP_API_URL}/${user.memberImage}`}
