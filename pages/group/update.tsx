...
const GroupUpdatePage = () => {
	...
	useEffect(() => {
		if (groupData?.getGroup) {
			...
			setImagePreview(`${REACT_APP_API_URL}/${group.groupImage}`);
		}
							...
							<div className="relative aspect-video w-full max-w-2xl mx-auto rounded-xl overflow-hidden bg-muted/50">
								{imagePreview ? (
									<>
										...
										>
											<div className="flex items-center gap-2 bg-background/90 text-foreground px-4 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200">
